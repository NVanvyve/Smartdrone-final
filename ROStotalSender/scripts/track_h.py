...
def usage(COM ="COM3", baud = 57600):
    ...
    parser.add_option("-H", "--Height", action="store_true", dest="height", help="Set Z axis at 0")
...
def setup_station_manual(options):
    ...
    # if options.height:
    raw_input('Put the drone on the floor to calibrate the height, direct the station to the prism and press <enter>')
    # else :
...
def compute_carthesian(phi,theta,radius):
    ...
    point_z = round((cos(theta) * radius) - HEIGHT,4)
...
def get_z():
    try:
        ...
        print(e.message)
        raw_input('Error : Height still set at 0\nPress <enter> to continue or <CTRL + C> to quit')
        return 0
